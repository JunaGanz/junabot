    ...
    juna.ev.on('group-participants.update', async (anu) => {
        ...
        try {
            ...
            for (let num of participants) {
                ...
                juna.sendMessage(anu.id, { caption: tks, location: { jpegThumbnail: jun }, footer: 'Jangan lupa intro Kak!', buttons: [{buttonId: `${prefix}menu`, buttonText: {displayText: '𝗪𝗘𝗟𝗖𝗢𝗠𝗘'}, type: 1}], headerType: 'LOCATION', mentions: [num] })
                ...
                juna.sendMessage(anu.id, { caption: tkks, location: { jpegThumbnail: atrei }, footer: 'Syukuran beban group keluar!', buttons: [{buttonId: `${prefix}menu`, buttonText: {displayText: '𝗚𝗢𝗢𝗗𝗕𝗬𝗘'}, type: 1}], headerType: 'LOCATION', mentions: [num] })
                }
            }
        } catch (err) {
            console.log(err)
